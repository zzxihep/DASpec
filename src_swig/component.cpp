...
/* ========== member function of class powerlaw_d ========== */
powerlaw_d::powerlaw_d(double r)
{
    npar = 4;
	ref = r;
	name = "powerlaw_d";
...
void powerlaw_d::info()
{
	...
	std::cout.precision(4);
	std::cout << "flux:" << " " << par[0] << " ";
    std::cout << "index1:" << " " << par[1] << std::endl;
    std::cout << "index2:" << " " << par[2] << std::endl;
    std::cout << "wave_edge:" << " " << par[3] << std::endl;
    std::cout.unsetf(std::ios::scientific); 
}

void powerlaw_d::calc(int num, double *x, double *y, double *p)
{
	for (int i = 0; i < npar; ++i) par[i] = p[i];
    double index1 = par[1];
    double index2 = par[2];
    ...
    if (xedge > ref){
        A = par[0];
        B = A * std::pow((xedge / ref), index1-index2);
    } else{
        B = par[0];
        A = B * std::pow((xedge / ref), index2-index1);
    }
    for (int i = 0; i < num; ++i){
        if (x[i] < xedge){
            y[i] += A * std::pow(x[i] / ref, index1);
        } else {
            y[i] += B * std::pow(x[i] / ref, index2);
        }
    }
}
